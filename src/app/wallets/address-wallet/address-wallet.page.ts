...
export class AddressWalletPage implements OnInit {
    ...
    setTimeout(async () => {
      try {
        ...
        if (!savedContacts || savedContacts.length == 0)
          throw new Error('Not found any contact');
        let localContacts = savedContacts.filter(
          (value) => !_.isEmpty(value.wallets)
        );
  ...
  navToDetail(id: string) {
    this.router.navigate(['/tabnav', 'wallets', 'send-request-multisig', id], {
