...
export class ContactProvider {
  ...
  create(
    ...
    name: string,
    address: string,
    chain: 'nem' | 'symbol' | 'bitcoin'
  ) {
    let sql =
      'INSERT INTO contact(owner, name, address, chain) VALUES(?,?,?,?)';
    return this.db.executeSql(sql, [owner, name, address, chain]);
  }

  createTable() {
    let sql =
      'CREATE TABLE IF NOT EXISTS contact(id INTEGER PRIMARY KEY AUTOINCREMENT, owner TEXT, name TEXT, address TEXT, chain TEXT)';
  ...
  searchContactName(
    ...
    return this.db
      .executeSql(sql, [owner, address, chain])
      .then((response) => {
        let contacts = [];
        for (let index = 0; index < response.rows.length; index++) {
          contacts.push(response.rows.item(index));
