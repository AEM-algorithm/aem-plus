...
export class PinProvider {
  ...
  public async showEnterPin(
    isVerifyBiometry = false,
  ...
  public async showDoublePinCheck(
    isShowForgotPin = true,
    ...
    if (pin) {
      ...
      if (mnemonics) {
        ...
        if (newPin) {
          ...
          const newPinNemWallets = this.newPinNemWallets(
            nemWallets,
            newPinHash,
            pinHash
          );
          const newPinSymbolWallets = this.newPinSymbolWallets(
            symbolWallets,
            newPinHash,
            pinHash
          );
          const newPinBitcoinWallets = this.newPinBitcoinWallets(
            bitcoinWallets,
            newPinHash,
            pinHash
          );
  ...
  private newPinMnemmonics(
    mnemonics: string[],
    newPassword: string,
    oldPassword?: string
  ) {
    return mnemonics.map((mnemonic) => {
      const decryptedMnemonic = oldPassword
        ? WalletProvider.decrypt(mnemonic, oldPassword)
        : mnemonic;
      const entropyMnemonic = oldPassword
        ? decryptedMnemonic
        : mnemonicToEntropy(decryptedMnemonic);
      const mnemonicEncrypted = WalletProvider.encrypt(
        entropyMnemonic,
        newPassword
      );
  ...
  private newPinNemWallets(
    nemWallets: NemWallet[],
    newPassword: string,
    oldPassword: string
  ) {
    return nemWallets.map((nemWallet) => {
      if (nemWallet.mnemonic) {
        nemWallet.mnemonic = this.newPinMnemmonics(
          [nemWallet.mnemonic],
          newPassword,
          oldPassword
        )[0];
      ...
      const newNemSimpleWallet = this.nem.createPrivateKeyWallet(
        'nemWallet',
        nemPrivateKey,
        newPassword
      );
      nemWallet.simpleWallet = newNemSimpleWallet.writeWLTFile();
      nemWallet.privateKey = JSON.stringify(
        newNemSimpleWallet.encryptedPrivateKey
      );
      return nemWallet;
    });
  }

  private newPinSymbolWallets(
    symbolWallets: SymbolWallet[],
    password: string,
    oldPassword: string
  ) {
    return symbolWallets.map((symbolWallet) => {
      if (symbolWallet.mnemonic) {
        symbolWallet.mnemonic = this.newPinMnemmonics(
          [symbolWallet.mnemonic],
          password,
          oldPassword
        )[0].toString();
      }
  ...
  private newPinBitcoinWallets(
    ...
    return bitcoinWallets.map((bitcoinWallet) => {
      if (bitcoinWallet.mnemonic) {
        bitcoinWallet.mnemonic = this.newPinMnemmonics(
      ...
      const newBitcoinSimpleWallet = this.bitcoin.createPrivateKeyWallet(
        bitcoinPrivateKey,
        password
      );
  ...
  public async checkMnemonic(verifingMnemonic: string): Promise<boolean> {
    const savedHashPin: AppPin = await this.appPasswordRepository.getPassword(
      'pin'
    );
    const decryptPin: string = PinProvider.decrypt(
      savedHashPin.encryptedPin,
      verifingMnemonic
    );
