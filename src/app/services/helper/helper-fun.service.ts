...
export class HelperFunService {
  ...
  onDayFilter(transactions: Transaction[]) {
    return transactions.filter((trans) =>
      this.isSameDay(new Date(trans.time), new Date())
    );
  }

  onWeekFilter(transactions: Transaction[]) {
    return transactions.filter((trans) =>
      this.isThisWeek(new Date(trans.time))
    );
  }

  onMonthFilter(transactions: Transaction[]) {
    return transactions.filter((trans) =>
      this.isThisMonth(new Date(trans.time), new Date())
    );
  }

  onYearFilter(transactions: Transaction[]) {
    return transactions.filter((trans) =>
      this.isThisYear(new Date(trans.time), new Date())
    );
  }
  ...
  dateRangeFilter(transactions: Transaction[], startDate: Date, endDate: Date) {
    return transactions.filter((trans) =>
      this.isInDateRange(new Date(trans.time), startDate, endDate)
    );
  }
  ...
  amountRangeFilter(
    ...
    return transactions.filter((trans) =>
      this.isInAmountRange(trans.amountCurrency, maxAmount, minAmount)
    );
  }

  amountRangeWithTypeFilter(
    transactions: Transaction[],
    maxAmount: number,
    minAmount: number,
    ...
    if (type === 'Currency') {
      ...
      return transactions.filter((trans) =>
        this.isInAmountRange(trans.amount, maxAmount, minAmount)
      );
    }
  }
}
