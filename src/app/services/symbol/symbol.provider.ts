...
export class SymbolProvider {
  ...
  private static readonly DEFAULT_ACCOUNT_PATH =
    environment.NETWORK_TYPE === 'MAIN_NET'
      ? `m/44'/4343'/0'/0'/0'`
      : `m/44'/1'/0'/0'/0'`;
  ...
  public SYMBOL_NETWORK_TYPE =
    environment.NETWORK_TYPE === 'MAIN_NET'
      ? NetworkType.MAIN_NET
      : NetworkType.TEST_NET;
  public symbolMosaicId =
    environment.NETWORK_TYPE === 'MAIN_NET'
      ? '6BED913FA20223F8'
      : '091F837E059AE13C';
  public readonly epochAdjustment = 1615853185;
  public readonly networkGenerationHash =
  ...
  public async setNodeSymbolWallet(walletId: string) {
    try {
      ...
      if (
        ...
        nodeWallet.nodes[0] != this.nodeList[0]
      )
  ...
  private async getNetworkMosaicId() {
    try {
      const networkCurrency = await this.repositoryFactory
        .getCurrencies()
        .toPromise();
  ...
  public async getBalance(
    ...
    { mosaic: Mosaic; info: MosaicInfo; namespaceNames: MosaicNames }[]
  > {
    try {
      ...
      const mosaicNames = await Promise.all(
      ...
      const names = await this.namespaceHttp
        .getMosaicsNames([new MosaicId(this.symbolMosaicId)])
        .toPromise();
      const info = await this.mosaicHttp
        .getMosaic(new MosaicId(this.symbolMosaicId))
        .toPromise();
      return [
        {
          mosaic: new Mosaic(
            new MosaicId(this.symbolMosaicId),
            UInt64.fromUint(0)
          ),
          info: info,
  ...
  async getXYMBalance(rawAddress: string): Promise<number> {
    try {
      ...
      const balanceByMosaicId = balances.find(
        (item) => item.mosaic.id.toHex() === this.symbolMosaicId
      );

      if (balanceByMosaicId) {
        const mosaic: Mosaic = balanceByMosaicId.mosaic;
        const mosaicInfo: MosaicInfo = balanceByMosaicId.info;
        const mathPow = Math.pow(10, mosaicInfo.divisibility);
  ...
  public async getAmountTxs(
    transferTxs: TransferTransaction,
    mosaicIdHex: string
  ): Promise<number> {
    try {
      if (transferTxs.type === TransactionType.TRANSFER) {
        const mosaic: Mosaic = this.getMosaicByTransaction(
          transferTxs,
          mosaicIdHex
        );
  ...
  public isHasMosaic(
    ...
    if (
      transferTxs.mosaics &&
      transferTxs.mosaics.find((mosaic) => mosaic.id.equals(mosaicId))
    ) {
      return true;
    }
    return false;
  ...
  public async getXYMPaidFee(
    transferTxs: TransferTransaction,
    mosaicIdHex: string
  ): Promise<number> {
    try {
      const blockInfo = await this.getBlockInfo(
        transferTxs.transactionInfo.height
      );
  ...
  isIncomingTxs(
    ...
    return (
      ...
      transaction.recipientAddress.equals(currentSignerAddress)
    );
  }
 ...
 */ /*
  ...
  public async confirmTransaction(
    ...
    try {
      const txStatus = await this.transactionStatusHttp
        .getTransactionStatus(signedTx.hash)
        .toPromise();
      if (transferTransaction.message.payload.length > 1023) {
        throw new Error('FAILURE_MESSAGE_TOO_LARGE');
  ...
  public async getMosaicsDefinition(mosaics: Mosaic[]): Promise<MosaicNames[]> {
    const mosaicIds = mosaics.map((_) =>
      _.id instanceof MosaicId ? _.id : null
    );
  ...
  public async getAllTransactionsFromAnAccount(
    ...
    const transactions = await this.transactionHttp
  ...
  public async getExportTransactionByPeriod(
    ...
    const transactionByPeriod = transactions.filter((txs) => {
      const date = TimeHelpers.getTransactionDate(
        txs.deadline,
        2,
        epochAdjustment,
        'l'
    ...
    for (const txs of transactionByPeriod) {
      ...
      if (
        ...
        this.isHasMosaic(transferTxs, this.symbolMosaicId)
      ) {
        const date = TimeHelpers.getTransactionDate(
          txs.deadline,
          2,
        ...
        const txsAmount = await this.getAmountTxs(
          transferTxs,
          this.symbolMosaicId
        );
  ...
  public async getAllTransactionsFromMosaicId(
    mosaicId: MosaicId
  ): Promise<Transaction[]> {
    const searchCriteria = {
      group: TransactionGroup.Confirmed,
   ...
   * @return Promise with account transactions
   */
  public async getFirstTransactionsFromAnAccount(
    address: Address
  ): Promise<Transaction[]> {
    const searchCriteria = {
      group: TransactionGroup.Confirmed,
   ...
   * @return Promise with account transactions
   */
  public async getUnconfirmedTransactionsFromAnAccount(
    address: Address
  ): Promise<Transaction[]> {
    const searchCriteria = {
  ...
  public async getEpochAdjustment(): Promise<number> {
    const epochAdjustment = await this.repositoryFactory
      .getEpochAdjustment()
      .toPromise();
